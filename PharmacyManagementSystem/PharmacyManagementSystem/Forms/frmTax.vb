...
Public Class frmTax
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If Len(Trim(txtPercentage.Text)) = 0 Then
            MessageBox.Show("Please enter %", "", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            txtPercentage.Focus()
            Exit Sub
        End If
        If txtAdditionalPercentage.Text = "" Then
            MessageBox.Show("Please enter additional %", "", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        ...
        Try
            ...
            cmd.ExecuteReader()
            con.Close()
            Dim st As String = "added the new Tax '" & txtTaxName.Text & "'"
    ...
    Private Sub DeleteRecord()
        ...
        Try
            ...
            If RowsAffected > 0 Then
                Dim st As String = "deleted the Tax '" & txtTaxName.Text & "'"
                LogFunc(lblUser.Text, st)
    ...
    Private Sub btnUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUpdate.Click
        Try
            ...
            cmd.ExecuteReader()
            con.Close()
            Dim st As String = "updated the TaxMaster '" & txtTaxName.Text & "' details"
            LogFunc(lblUser.Text, st)
    ...
    Private Sub txtFirstName_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtSearchByTaxName.TextChanged
        Try
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT RTRIM(TaxName),RTRIM(taxtype),Percentage,AdditionalPercentage from TaxMaster where TaxName like '%" & txtSearchByTaxName.Text & "%' order by TaxName", con)
            rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            dgw.Rows.Clear()
            While (rdr.Read() = True)
                dgw.Rows.Add(rdr(0), rdr(1), rdr(2), rdr(3))
