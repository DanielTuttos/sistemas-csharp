...
Public Class frmSqlServerSetting
            ...
            If (con.State = ConnectionState.Open) Then
                If MsgBox("It will create the DB and configure the sql server, Do you want to proceed?", MsgBoxStyle.YesNo + MsgBoxStyle.Information) = MsgBoxResult.Yes Then
                    ...
                    Using sw As StreamWriter = New StreamWriter(Application.StartupPath & "\SQLSettings.dat")
                        If cmbAuthentication.SelectedIndex = 0 Then
                            sw.WriteLine("Data Source=" & cmbServerName.Text.Trim & ";Initial Catalog=PharmacyDB;Integrated Security=True")
                            sw.Close()
                        End If
                        If cmbAuthentication.SelectedIndex = 1 Then
                            sw.WriteLine("Data Source=" & cmbServerName.Text.Trim & ";Initial Catalog=PharmacyDB;User ID=" & txtUserName.Text.Trim & ";Password=" & txtPassword.Text & "")
                            sw.Close()
                        End If
                        CreateDB()
                    End Using
    ...
    Sub CreateDB()
        Try
            con = New SqlConnection("Data source=" & cmbServerName.Text & ";Initial Catalog=master;Integrated Security=True;")
            con.Open()
            Dim cb2 As String = "Select * from sysdatabases where name='PharmacyDB'"
            ...
            If rdr.Read() Then
                con = New SqlConnection("Data source=" & cmbServerName.Text & ";Initial Catalog=master;Integrated Security=True;")
                con.Open()
                Dim cb1 As String = "Drop Database PharmacyDB"
                ...
                con = New SqlConnection("Data source=" & cmbServerName.Text & ";Initial Catalog=master;Integrated Security=True;")
                con.Open()
                Dim cb As String = "Create Database PharmacyDB"
            ...
            Else
                con = New SqlConnection("Data source=" & cmbServerName.Text & ";Initial Catalog=master;Integrated Security=True;")
                con.Open()
                Dim cb3 As String = "Create Database PharmacyDB"
