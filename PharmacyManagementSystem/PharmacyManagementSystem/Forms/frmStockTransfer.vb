...
Public Class frmStockTransfer
    Sub FillStore()
        Try
            ...
            adp.SelectCommand = New SqlCommand("SELECT distinct RTRIM(StoreName) FROM Store", CN)
            ds = New DataSet("ds")
            adp.Fill(ds)
            dtable = ds.Tables(0)
            cmbStore.Items.Clear()
            For Each drow As DataRow In dtable.Rows
                cmbStore.Items.Add(drow(0).ToString())
            Next
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        Try
            If Len(Trim(cmbStore.Text)) = 0 Then
            ...
            For Each row As DataGridViewRow In dgw.Rows
                con = New SqlConnection(cs)
                con.Open()
                Dim ctX As String = "select Store,ProductID,ExpiryDate from temp_Stock_Store where Store=@d1 and ProductID=@d2 and ExpiryDate=@d3"
                cmd = New SqlCommand(ctX)
                cmd.Connection = con
                cmd.Parameters.AddWithValue("@d1", cmbStore.Text)
                cmd.Parameters.AddWithValue("@d2", Val(row.Cells(1).Value))
                cmd.Parameters.AddWithValue("@d3", row.Cells(3).Value)
                rdr = cmd.ExecuteReader()
                If (rdr.Read()) Then
                    con = New SqlConnection(cs)
                    con.Open()
                    Dim cb2 As String = "Update Temp_Stock_Store set Qty=Qty + " & Val(row.Cells(4).Value) & " where Store=@d1 and ProductID=@d2 and ExpiryDate=@d3"
                    cmd = New SqlCommand(cb2)
                    cmd.Connection = con
                    cmd.Parameters.AddWithValue("@d1", cmbStore.Text)
                    cmd.Parameters.AddWithValue("@d2", Val(row.Cells(1).Value))
                ...
                Else
                    con = New SqlConnection(cs)
                    con.Open()
                    Dim cb3 As String = "Insert Into Temp_Stock_Store(Store,ProductID,Qty,ExpiryDate) values (@d1,@d2,@d3,@d4)"
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb As String = "insert into StockTransfer(ST_ID, Date,Store) VALUES (@d1,@d2,@d3)"
            cmd = New SqlCommand(cb)
            cmd.Parameters.AddWithValue("@d1", Val(txtID.Text))
            cmd.Parameters.AddWithValue("@d2", dtpDate.Value.Date)
            cmd.Parameters.AddWithValue("@d3", cmbStore.Text)
            cmd.Connection = con
            cmd.ExecuteNonQuery()
            con.Close()
            con = New SqlConnection(cs)
    ...
    Private Sub DeleteRecord()
        Try
            ...
            For Each row As DataGridViewRow In dgw.Rows
                con = New SqlConnection(cs)
                con.Open()
                Dim ct As String = "select Store,ProductID,ExpiryDate from temp_Stock_Store where Store=@d1 and ProductID=@d2 and ExpiryDate=@d3"
                cmd = New SqlCommand(ct)
                cmd.Connection = con
                cmd.Parameters.AddWithValue("@d1", cmbStore.Text)
                cmd.Parameters.AddWithValue("@d2", Val(row.Cells(1).Value))
    ...
    Public Sub Getdata()
        Try
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT RTRIM(WareHouse),RTRIM(ProductID),RTRIM(ProductName),RTRIM(PurchaseUnit),RTRIM(ExpiryDate),Qty from Temp_Stock,Product where Temp_Stock.ProductID=Product.PID and Qty > 0 order by ProductName", con)
            rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
    ...
    Private Sub btnPrint_Click(sender As System.Object, e As System.EventArgs) Handles btnPrint.Click
        Try
            ...
            MyCommand.CommandText = "SELECT StockTransfer.ST_ID, StockTransfer.Date, StockTransfer.Store, StockTransfer_Join.STJ_ID, StockTransfer_Join.StockTransferID, StockTransfer_Join.Warehouse, StockTransfer_Join.ProductID,StockTransfer_Join.ExpiryDate, StockTransfer_Join.Qty, Product.PID, Product.ProductCode, Product.ProductName, Product.Category, Product.PurchaseUnit FROM StockTransfer INNER JOIN StockTransfer_Join ON StockTransfer.ST_ID = StockTransfer_Join.StockTransferID INNER JOIN Product ON StockTransfer_Join.ProductID = Product.PID where ST_ID=" & Val(txtID.Text) & " order by StockTransfer.Date"
