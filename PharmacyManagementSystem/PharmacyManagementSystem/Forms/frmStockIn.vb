...
Public Class frmCurrentStock
    ...
    Public Sub Getdata()
        Try
            ...
            con.Open()
            cmd = New SqlCommand("SELECT PID, RTRIM(Product.ProductCode),RTRIM(ProductName),RTRIM(Barcode),(PurchaseCost),(SalesCost),(DiscountPer),(Percentage),(AdditionalPercentage),RTRIM(ExpiryDate),Qty from Temp_Stock_Store,Product,TaxMaster where Product.PID=Temp_Stock_Store.ProductID and TaxMaster.TaxName=Product.VAtCommodity and Qty > 0  order by ProductName", con)
    ...
    Private Sub txtProductName_TextChanged(sender As System.Object, e As System.EventArgs) Handles txtProductName.TextChanged
        Try
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT PID, RTRIM(Product.ProductCode),RTRIM(ProductName),RTRIM(Barcode),(PurchaseCost),(SalesCost),(DiscountPer),(Percentage),(AdditionalPercentage),RTRIM(ExpiryDate),Qty from Temp_Stock_Store,Product,TaxMaster where Product.PID=Temp_Stock_Store.ProductID and TaxMaster.TaxName=Product.VAtCommodity and Qty > 0 and ProductName like '%" & txtProductName.Text & "%' order by ProductName", con)
            rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            dgw.Rows.Clear()
            While (rdr.Read() = True)
                dgw.Rows.Add(rdr(0), rdr(1), rdr(2), rdr(3), rdr(4), rdr(5), rdr(6), rdr(7), rdr(8), rdr(9), rdr(10))
